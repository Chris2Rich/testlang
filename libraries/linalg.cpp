...
struct Mat {
  ...
  double dot_col(long j, const Mat &other, long oj) const {
    double s = 0.0;
    for (long i = 0; i < rows; ++i)
      s += at(i, j) * other.at(i, oj);
    return s;
  }

  double norm_col(long j) const { return std::sqrt(dot_col(j, *this, j)); }
...
static bool require_2d(const Value *v, const char *fn) {
  if (!v->is_array || v->shape.size() != 2) {
    std::cerr << "Runtime Error: " << fn << " requires a 2-D matrix"
              << std::endl;
    return false;
  }
  return true;
...
static bool require_square(const Value *v, const char *fn) {
  ...
  if (v->shape[0] != v->shape[1]) {
    std::cerr << "Runtime Error: " << fn << " requires a square matrix"
              << std::endl;
    return false;
  }
  return true;
...
static void qr_decompose(const Mat &A, Mat &Q, Mat &R) {
  ...
  for (long j = 0; j < n; ++j) {
    ...
    if (norm < LINALG_EPS) {
      ...
      // Set R's diagonal to strictly 0 so that A = QR reconstructs
      // perfectly without floating-point garbage later.
      R.at(j, j) = 0.0;

      // Find an arbitrary unit vector orthogonal to previous Q columns.
      // We test standard basis vectors (e_0, e_1, ... e_m-1) one by one.
      for (long p = 0; p < m; ++p) {
        // 1. Temporarily set Q's j-th column to the basis vector e_p
        for (long i = 0; i < m; ++i)
          Q.at(i, j) = (i == p) ? 1.0 : 0.0;

        // 2. Project out all previously computed columns (q_0 to q_{j-1})
        for (long c = 0; c < j; ++c) {
          double proj = Q.dot_col(c, Q, j); // <q_c, e_p>
          for (long i = 0; i < m; ++i)
            Q.at(i, j) -= proj * Q.at(i, c);
        }

        // 3. Check if the resulting vector survived (isn't zero)
        double cand_norm = Q.norm_col(j);
        if (cand_norm > LINALG_EPS) {
          // 4. Normalize it and accept it as our new q_j
          for (long i = 0; i < m; ++i)
            Q.at(i, j) /= cand_norm;
          break; // Successfully found our missing column!
        }
      }
    } else {
      // --- NORMAL CASE ---
    ...
    // (This happens normally regardless of whether q_j was generated or not)
...
extern "C" void inverse() {
  ...
  if (!require_square(v, "linalg.inverse")) {
    delete v;
    return;
  }
...
extern "C" void eigen() {
  ...
  if (!require_square(v, "linalg.eigen")) {
    delete v;
    return;
  }
